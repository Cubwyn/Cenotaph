...
$generator = Join-Path $PSScriptRoot "generate_prototype_models.py"
$launchers = @()
...
if ($null -eq $python) {
    throw "Python 3 was not found. Install Python 3 or run the generator through the bundled Codex workspace runtime."
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Prototype model generation failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
