...
impl GameConfig {
    ...
    /// Load key bindings from the old config system for compatibility
    pub fn load_keys() -> std::collections::HashMap<String, Option<winit::keyboard::KeyCode>> {
        ...
        let config_str = match std::fs::read_to_string("config/Config.toml")
            .or_else(|_| std::fs::read_to_string("config/config.toml")) {
            Ok(s) => s,
            Err(_) => {
                eprintln!("Warning: Could not find config/Config.toml. Using default keys.");
                return default_keys;
            }
        };

        let raw: toml::Value = match toml::from_str(&config_str) {
            Ok(v) => v,
            Err(_) => {
                eprintln!("Warning: Failed to parse Config.toml. Using default keys.");
                return default_keys;
            }
        };
        ...
        if let Some(keybindings) = raw.get("keybindings") {
            if let Some(keybindings_table) = keybindings.as_table() {
                ...
                for (action, key_str) in keybindings_table.iter() {
                    keys.insert(action.clone(), parse_key(key_str.as_str().unwrap_or("")));
                }
                return keys;
            }
        }
...
// Remaining configs focused strictly on movement/physics
